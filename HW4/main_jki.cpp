...
#include <chrono>
#include <fstream>
#include "jki_loop.hpp"
#include "kij_loop.hpp"

int main()
{
    // DOUBLE
    std::cout << "Calculating doubles [JKI]" << std::endl;

    std::ofstream out("jki_double_O3.csv");
    out << "n,flops_per_sec\n";
    ...
    for (int n = 2; n <= 512; ++n) {
        ...
        out << n << "," << flops_per_sec << "\n";
        std::cout << "[DOUBLE, JKI, O3] Average Time = " << avg_time << ", n = " << n << ", Average FLOPs/sec = " << flops_per_sec << "\n";
    }
    ...
    std::cout << "Now onto floats [JKI]" << std::endl;

    std::ofstream out2("jki_float_O3.csv");
    ...
    for (int n = 2; n <= 512; ++n) {
        ...
        out2 << n << "," << flops_per_sec << "\n";
        std::cout << "[FLOAT, JKI, O3] Average Time = " << avg_time << ", n = " << n << ", Average FLOPs/sec = " << flops_per_sec << "\n";
    }
